...
export default class EventVariables {
  ...
  oldmessage(oldmessage: Message) {
    return this.replace(/\[OLDMESSAGE\]/g, oldmessage.content)
  }

  newmessage(newmessage: Message) {
    return this.replace(/\[NEWMESSAGE\]/g, newmessage.content)
  }
  ...
  userName(user: User) {
    return this.replace(/\[USER_NAME\]/g, `${user.username}#${user.discriminator}`);
  }

  userAvatar(user: User) {
    return this.replace(/\[USER_AVATAR\]/g, user.avatarURL());
  }

  staff(user: User) {
    return this.replace(/\[STAFF\]/g, `<@!${user.id}>`);
  }
